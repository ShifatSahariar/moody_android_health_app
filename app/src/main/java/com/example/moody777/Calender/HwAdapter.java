    ...
    public void getPositionList(String date,final Activity act){
        ...
        for (int j=0; j<len; j++){
            if (HomeCollection.date_collection_arr.get(j).date.equals(date)){
                ...
                maplist.put("hours",HomeCollection.date_collection_arr.get(j).TOTAL_HOURS);
                maplist.put("sleepType",HomeCollection.date_collection_arr.get(j).SLEEP_TYPE);
                maplist.put("awakeTime",HomeCollection.date_collection_arr.get(j).AWAKE_NIGHT_TIME);
                maplist.put("awakeType",HomeCollection.date_collection_arr.get(j).SLEEP_AWAKE_TYPE);
                maplist.put("moodWas",HomeCollection.date_collection_arr.get(j).MOOD_YESTERDAY_WAS);
                maplist.put("exTime",HomeCollection.date_collection_arr.get(j).EXERCISE_TIME_YESTERDAY_WAS);
                maplist.put("exType",HomeCollection.date_collection_arr.get(j).EXERCISE_TYPE_YESTERDAY_WAS);
    ...
    private ArrayList<Dialogpojo> getMatchList(String detail) {
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                pojo.setTotalHours(jsonObject.optString("hours"));
                pojo.setSleepType(jsonObject.optString("sleepType"));
                pojo.setAwakeNightTime(jsonObject.optString("awakeTime"));
                pojo.setAwakeType(jsonObject.optString("awakeType"));
                pojo.setMoodWas(jsonObject.optString("moodWas"));
                pojo.setExTime(jsonObject.optString("exTime"));
                pojo.setExType(jsonObject.optString("exType"));
